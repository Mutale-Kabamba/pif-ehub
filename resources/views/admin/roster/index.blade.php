...
@section('title', 'Candidates & Panelists Roster — Play It Forward')
...
<div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; margin-bottom:20px;">
    <div>
        <a href="{{ route('admin.dashboard') }}" style="color:var(--text-secondary); text-decoration:none; font-size:0.85rem; display:inline-flex; align-items:center; gap:6px; margin-bottom:4px;">
            ← Back to Dashboard
        </a>
    ...
    <div style="display:flex; gap:8px; flex-wrap:wrap;">
        ...
        <a href="{{ route('admin.roster.index', ['tab' => 'import']) }}" class="btn btn-primary btn-sm" style="display:inline-flex; align-items:center; gap:6px;">
            📥 Import from Excel
        </a>
    </div>
</div>
...
<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px, 1fr)); gap:12px; margin-bottom:20px;">
    <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:12px 16px; border-left:4px solid var(--green-primary);">
        ...
        <div style="font-size:1.5rem; font-weight:800; color:var(--text-primary); margin-top:2px;">{{ $totalCandidates }}</div>
    </div>
    <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:12px 16px; border-left:4px solid #6C63FF;">
        ...
        <div style="font-size:1.5rem; font-weight:800; color:var(--text-primary); margin-top:2px;">{{ $panelACount }}</div>
    </div>
    <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:12px 16px; border-left:4px solid #1A7F4F;">
        ...
        <div style="font-size:1.5rem; font-weight:800; color:var(--text-primary); margin-top:2px;">{{ $panelBCount }}</div>
    </div>
    <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:12px 16px; border-left:4px solid #2563EB;">
        ...
        <div style="font-size:1.5rem; font-weight:800; color:var(--text-primary); margin-top:2px;">{{ $totalPanelists }}</div>
    </div>
</div>

{{-- Navigation Tabs --}}
<div class="tabs" style="margin-bottom:16px;">
    <a href="{{ route('admin.roster.index', ['tab' => 'candidates']) }}" class="tab {{ $activeTab === 'candidates' ? 'active' : '' }}">
        🧑‍🎓 Candidates ({{ $totalCandidates }})
    </a>
    <a href="{{ route('admin.roster.index', ['tab' => 'panelists']) }}" class="tab {{ $activeTab === 'panelists' ? 'active' : '' }}">
        👥 Panelists ({{ $totalPanelists }})
    </a>
    <a href="{{ route('admin.roster.index', ['tab' => 'import']) }}" class="tab {{ $activeTab === 'import' ? 'active' : '' }}">
...
@if($activeTab === 'candidates')
    <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:16px; margin-bottom:20px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; margin-bottom:16px;">
            <form method="GET" action="{{ route('admin.roster.index') }}" style="display:flex; gap:8px; align-items:center; flex-wrap:wrap; margin:0;">
                <input type="hidden" name="tab" value="candidates">
                <input type="text" name="q_candidate" value="{{ request('q_candidate') }}" placeholder="Search candidate name..."
                ...
                <select name="panel_candidate" onchange="this.form.submit()" style="padding:6px 10px; font-size:0.85rem; border:1px solid var(--border); border-radius:var(--radius-sm);">
                    ...
                    <option value="A" {{ request('panel_candidate') === 'A' ? 'selected' : '' }}>Panel A</option>
                    <option value="B" {{ request('panel_candidate') === 'B' ? 'selected' : '' }}>Panel B</option>
                    <option value="unassigned" {{ request('panel_candidate') === 'unassigned' ? 'selected' : '' }}>Unassigned</option>
                </select>
                ...
                <select name="gender_candidate" onchange="this.form.submit()" style="padding:6px 10px; font-size:0.85rem; border:1px solid var(--border); border-radius:var(--radius-sm);">
                    ...
                    <option value="Female" {{ request('gender_candidate') === 'Female' ? 'selected' : '' }}>Female</option>
                    <option value="Male" {{ request('gender_candidate') === 'Male' ? 'selected' : '' }}>Male</option>
                </select>

                <button type="submit" class="btn btn-outline btn-sm">Filter</button>
                @if(request('q_candidate') || request('panel_candidate') || request('gender_candidate'))
                    <a href="{{ route('admin.roster.index', ['tab' => 'candidates']) }}" class="btn btn-ghost btn-sm">Clear</a>
                @endif
            </form>
            ...
            <div style="display:flex; gap:8px;">
                ...
                <a href="{{ route('admin.imports.template', ['type' => 'candidates']) }}" class="btn btn-outline btn-sm">
                    📥 Template (.xlsx)
                </a>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach($candidates as $idx => $cand)
                            <tr>
                                ...
                                <td style="padding:8px 12px; text-align:center;">
                                    @if($cand->literacyScore)
                                        <span style="font-weight:700; color:var(--green-dark);">
                                            {{ $cand->literacyScore->total_score }} / 20
                                        </span>
                                    @else
                                ...
                                <td style="padding:8px 12px; text-align:center;">
                                    @if($cand->panel_scores_count > 0)
                                        <span class="badge badge-teal" style="font-size:0.75rem; padding:2px 8px;">
                                            {{ $cand->panel_scores_count }} evaluation{{ $cand->panel_scores_count > 1 ? 's' : '' }}
                                        </span>
                                    @else
                                ...
                                <td style="padding:8px 12px; text-align:right;">
                                    <div style="display:inline-flex; gap:6px;">
                                        <button type="button" class="btn btn-ghost btn-sm" style="padding:4px 8px; font-size:0.8rem;"
                                                onclick="editCandidate({{ json_encode($cand) }})">
                                            ✏️ Edit
                                        </button>
                                        <form action="{{ route('admin.roster.candidates.destroy', $cand->id) }}" method="POST"
                                              onsubmit="return confirm('Are you sure you want to delete {{ addslashes($cand->name) }}? All linked evaluation scores will also be removed.');"
    ...
    <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:16px; margin-bottom:20px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; margin-bottom:16px;">
            <form method="GET" action="{{ route('admin.roster.index') }}" style="display:flex; gap:8px; align-items:center; flex-wrap:wrap; margin:0;">
                ...
                <input type="text" name="q_panelist" value="{{ request('q_panelist') }}" placeholder="Search panelist name or email..."
                ...
                <select name="panel_panelist" onchange="this.form.submit()" style="padding:6px 10px; font-size:0.85rem; border:1px solid var(--border); border-radius:var(--radius-sm);">
                    <option value="">All Panels</option>
                    <option value="A" {{ request('panel_panelist') === 'A' ? 'selected' : '' }}>Panel A</option>
                    <option value="B" {{ request('panel_panelist') === 'B' ? 'selected' : '' }}>Panel B</option>
                    <option value="cover" {{ request('panel_panelist') === 'cover' ? 'selected' : '' }}>Cover / Observer</option>
                </select>

                <button type="submit" class="btn btn-outline btn-sm">Filter</button>
                @if(request('q_panelist') || request('panel_panelist'))
                    <a href="{{ route('admin.roster.index', ['tab' => 'panelists']) }}" class="btn btn-ghost btn-sm">Clear</a>
                @endif
            </form>
            ...
            <div style="display:flex; gap:8px;">
                ...
                <a href="{{ route('admin.imports.template', ['type' => 'panelists']) }}" class="btn btn-outline btn-sm">
                    📥 Template (.xlsx)
                </a>
            </div>
        </div>
        ...
        <div style="overflow-x:auto;">
            <table class="data-table" style="width:100%; font-size:0.85rem;">
                ...
                <tbody>
                    @foreach($panelists as $idx => $p)
                        <tr>
                            <td style="padding:8px 12px; color:var(--text-muted);">{{ $idx + 1 }}</td>
                            <td style="padding:8px 12px; font-weight:600; color:var(--text-primary);">
                                {{ $p->panelist_name ?: $p->name }}
                            </td>
                            ...
                            <td style="padding:8px 12px; text-align:center;">
                                <span class="badge badge-teal" style="font-size:0.75rem; padding:2px 8px;">
                                    {{ $p->panel_scores_count }} scored
                                </span>
                            </td>
                            <td style="padding:8px 12px; text-align:right;">
                                <div style="display:inline-flex; gap:6px;">
                                    <button type="button" class="btn btn-ghost btn-sm" style="padding:4px 8px; font-size:0.8rem;"
                                            onclick="editPanelist({{ json_encode($p) }})">
                                        ✏️ Edit
                                    </button>
                                    @if($p->id !== auth()->id() && $p->id !== session('admin_user_id'))
                                        <form action="{{ route('admin.roster.panelists.destroy', $p->id) }}" method="POST"
                                              onsubmit="return confirm('Are you sure you want to remove panelist {{ addslashes($p->name) }}?');"
                                              style="display:inline; margin:0;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-ghost btn-sm" style="padding:4px 8px; font-size:0.8rem; color:#dc2626;">
...
{{-- ========================================================= --}}
{{-- TAB 3: IMPORT RESULTS & ROSTERS FROM EXCEL               --}}
{{-- ========================================================= --}}
@if($activeTab === 'import')
    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(320px, 1fr)); gap:16px; margin-bottom:24px;">

        {{-- 1. Interview Panel Scores Import Card --}}
        <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:20px; display:flex; flex-direction:column; justify-content:space-between; border-top:4px solid #2563EB;">
            ...
            <div>
                <form action="{{ route('admin.imports.interview-scores') }}" method="POST" enctype="multipart/form-data" style="margin-bottom:10px;">
                    @csrf
                ...
                <div style="text-align:center;">
                    <a href="{{ route('admin.imports.template', ['type' => 'interview-scores']) }}" style="font-size:0.8rem; color:var(--green-dark); text-decoration:none;">
        ...
        {{-- 2. Literacy Assessment Scores Import Card --}}
        <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:20px; display:flex; flex-direction:column; justify-content:space-between; border-top:4px solid var(--green-primary);">
            ...
            <div>
                <form action="{{ route('admin.imports.literacy-scores') }}" method="POST" enctype="multipart/form-data" style="margin-bottom:10px;">
                    @csrf
                    <input type="hidden" name="redirect_to" value="roster">
                    <div style="margin-bottom:10px;">
                        <input type="file" name="file" accept=".xlsx,.xls,.csv" required
                ...
                <div style="text-align:center;">
                    <a href="{{ route('admin.imports.template', ['type' => 'literacy-scores']) }}" style="font-size:0.8rem; color:var(--green-dark); text-decoration:none;">
                        📥 Download Sample Template (.xlsx)
                    </a>
                </div>
            </div>
        ...
        {{-- 3. Bulk Candidates Roster Import Card --}}
        <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:20px; display:flex; flex-direction:column; justify-content:space-between; border-top:4px solid #6C63FF;">
            ...
            <div>
                <form action="{{ route('admin.imports.candidates') }}" method="POST" enctype="multipart/form-data" style="margin-bottom:10px;">
                    @csrf
                    <div style="margin-bottom:10px;">
                        <input type="file" name="file" accept=".xlsx,.xls,.csv" required
                               style="font-size:0.82rem; width:100%; padding:6px; border:1px dashed var(--border); border-radius:var(--radius-sm); background:#fff;">
                ...
                <div style="text-align:center;">
                    <a href="{{ route('admin.imports.template', ['type' => 'candidates']) }}" style="font-size:0.8rem; color:var(--green-dark); text-decoration:none;">
                        📥 Download Sample Template (.xlsx)
                    </a>
                </div>
            </div>
        ...
        <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:20px; display:flex; flex-direction:column; justify-content:space-between; border-top:4px solid #7c3aed;">
            ...
            <div>
                <form action="{{ route('admin.imports.panelists') }}" method="POST" enctype="multipart/form-data" style="margin-bottom:10px;">
                    @csrf
                    <div style="margin-bottom:10px;">
                        <input type="file" name="file" accept=".xlsx,.xls,.csv" required
                               style="font-size:0.82rem; width:100%; padding:6px; border:1px dashed var(--border); border-radius:var(--radius-sm); background:#fff;">
        ...
        {{-- 5. Dynamic Assessments, Interviews & Surveys Results --}}
                    ...
                    <select id="dynamicAssessmentSelect" class="form-control" style="font-size:0.85rem;" onchange="updateDynamicAssessmentActions(this.value)">
                        ...
                        @else
                            @foreach($assessments as $asm)
                                <option value="{{ $asm->id }}" data-type="{{ $asm->type }}">
                                    [{{ ucfirst($asm->type) }}] {{ $asm->title }} ({{ $asm->questions_count }} Qs, {{ $asm->candidates_count }} cands)
                                </option>
                            @endforeach
                        @endif
                    </select>
        ...
        {{-- 6. Baseline & Endline Survey Responses Import Card --}}
        <div style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius-md); padding:20px; display:flex; flex-direction:column; justify-content:space-between; border-top:4px solid #EC4899;">
            ...
            <div>
                <form action="{{ route('admin.imports.survey-responses') }}" method="POST" enctype="multipart/form-data" style="margin-bottom:10px;">
                    @csrf
                    <div style="margin-bottom:10px;">
                        <input type="file" name="file" accept=".xlsx,.xls,.csv" required
                               style="font-size:0.82rem; width:100%; padding:6px; border:1px dashed var(--border); border-radius:var(--radius-sm); background:#fff;">
                ...
                <div style="display:flex; justify-content:center; gap:12px; font-size:0.8rem;">
                    <a href="{{ route('admin.imports.template', ['type' => 'survey-responses', 'format' => 'xlsx']) }}" style="color:var(--green-dark); text-decoration:none;">
                        📥 Template (.xlsx)
                    </a>
                    <span style="color:var(--border);">|</span>
                    <a href="{{ route('admin.imports.template', ['type' => 'survey-responses', 'format' => 'csv']) }}" style="color:var(--green-dark); text-decoration:none;">
                        📥 Template (.csv)
                    </a>
                </div>
            </div>
...
{{-- ========================================================= --}}
{{-- MODALS: ADD & EDIT CANDIDATES / PANELISTS                 --}}
{{-- ========================================================= --}}
...
<div id="addCandidateModal" class="pif-modal" style="display:none;">
    ...
    <div class="pif-modal-card">
        ...
        <form action="{{ route('admin.roster.candidates.store') }}" method="POST">
            @csrf
...
<div id="addPanelistModal" class="pif-modal" style="display:none;">
    ...
    <div class="pif-modal-card">
        ...
        <form action="{{ route('admin.roster.panelists.store') }}" method="POST">
            @csrf
            <div class="form-group" style="margin-bottom:12px;">
