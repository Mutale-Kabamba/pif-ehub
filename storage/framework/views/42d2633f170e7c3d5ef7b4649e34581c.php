...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const labels = <?php echo json_encode(array_map(function($q) {
        // Extract first ~40 chars as short label
        return substr($q, 0, 40) . (strlen($q) > 40 ? '...' : '');
    }, $quantQuestions ?? [])); ?>;

    // Chart data passed from controller
    const baselineData = <?php echo json_encode(array_map(function($key) use ($avgScores) {
        return $avgScores[$key]['baseline'] ?? 0;
    }, array_keys($quantQuestions ?? []))); ?>;

    const endlineData = <?php echo json_encode(array_map(function($key) use ($avgScores) {
...
<?php /**PATH C:\Users\mukuk\Downloads\Kimi_Agent_Composer Discover Error\project\resources\views/admin/partials/analytics.blade.php ENDPATH**/ ?>