...
<div style="display: flex; justify-content: flex-end; margin: 0 0 12px;">
    <a href="<?php echo e(route('admin.survey.export')); ?>" class="btn" style="text-decoration: none; padding: 8px 14px; font-size: 0.9rem;">
...
<div class="metric-cards">
    <div class="metric-card">
        <div class="value"><?php echo e($totalResponses ?? 0); ?></div>
        <div class="label">Total Submitted Survey Forms</div>
    </div>
    <div class="metric-card">
        <div class="value"><?php echo e($baselineCount ?? 0); ?></div>
        <div class="label">Baseline Entries (Day 1)</div>
    </div>
    <div class="metric-card">
        <div class="value"><?php echo e($endlineCount ?? 0); ?></div>
            ...
            <?php $__currentLoopData = ($quantQuestions ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $question): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr>
                    ...
                    <td style="text-align: center; border-bottom: 1px solid #f0f0f0; padding: 10px 8px;">
                        <?php echo e(number_format((float) (($avgScores[$key]['baseline'] ?? 0)), 2)); ?>

                    </td>
                    <td style="text-align: center; border-bottom: 1px solid #f0f0f0; padding: 10px 8px;">
                        <?php echo e(number_format((float) (($avgScores[$key]['endline'] ?? 0)), 2)); ?>
...
(function() {
    ...
    const labels = <?php echo json_encode(array_values(array_map(function($q) {
        return substr($q, 0, 40) . (strlen($q) > 40 ? '...' : '');
    }, $quantQuestions ?? []))); ?>;

    const baselineData = <?php echo json_encode(array_values(array_map(function($key) use ($avgScores) {
        return $avgScores[$key]['baseline'] ?? 0;
    }, array_keys($quantQuestions ?? [])))); ?>;

    const endlineData = <?php echo json_encode(array_values(array_map(function($key) use ($avgScores) {
...
<?php /**PATH C:\Users\mukuk\Documents\GitHub\pif-ehub\resources\views/admin/partials/analytics.blade.php ENDPATH**/ ?>